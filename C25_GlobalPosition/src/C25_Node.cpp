 ...
 * Received from GPS and camera sensors. data is relative to the head or body
 *
...
public:
	 ...
	 */
	  C25_Node(std::string left_camera,std::string right_camera) :
		it_(nh_),
		...
		sync( MySyncPolicy( 10 ), left_image_sub_, right_image_sub_ )
	  {
		...
		sync.registerCallback( boost::bind( &C25_Node::callback, this, _1, _2 ) );  //Specifying what to do with the data
		service = nh_.advertiseService("C25", &C25_Node::proccess, this); //Specifying what to do when a reconstructed 3d scene is requested
		ROS_INFO("service on\n");
	  }
...
int main(int argc, char **argv)
{
  ...
  if(argc!=3){
	  printf("usage: C25_module <left camera topic> <right camera topic>");
  }
  C25_Node my_node(argv[1],argv[2]);
