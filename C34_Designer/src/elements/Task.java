...
public class Task extends GElement implements View.ChangesListener {
	...
	public String getText() {
		...
		if (text.length() > shortTextLen)
			return text.substring(0, shortTextLen - 3) + "...";
		return text;
	}
	...
	private class Seq extends Border {
		...
		public Seq(int x, int y, int w, int h) {
			...
			_x = new int[] { x, (int) (x + w - t.x), (int) (x + w - t.x),
					x + w, (int) (x + w - t.x), (int) (x + w - t.x), x, x };
			_y = new int[] { (int) (y + t.y), (int) (y + t.y), y,
	...
	private class Par extends Border {
		...
		public Par(int x, int y, int w, int h) {
			...
			_x = new int[] { x + t.getIntX(), x + w + t.getIntX(),
					x + w - t.getIntX(), x - t.getIntX() };
			_y = new int[] { y, y, y + h, y + h };
		}
	...
	private class Swi extends Border {
		...
		public void paint(Graphics2D g) {
			...
			if (w >= 5 * 5) {
				int kk, nn;
				for (kk = 3, nn = 3; w / kk >= 5; nn = kk, kk += 2)
					;
	...
	public void paint(Graphics2D g) {
		...
		Font f = new Font(font.getFamily(), font.getStyle(),
				(int) (fontsize * view.zoom));
		g.setFont(f);
		...
		if (seqNumber > 0) {
			f = new Font(font.getFamily(), font.getStyle(),
					(int) (fontsize * 0.8 * view.zoom));
			g.setFont(f);
			g.setPaint(Color.blue);
			tdim = new Vec(getTextSize(g, "" + seqNumber)).scale(0.5)
					.getDimension();
			cnt = getLocation().getPoint();
			drawString(g, "" + seqNumber, cnt.x - tdim.width, cnt.y
					- tdim.height);
		...
		if (property.collapsed) {
			...
			g.drawLine(cnt.x + dim.getIntX() / 2,
					cnt.y + (int) (view.zoom * 2), cnt.x + dim.getIntX() / 2,
					cnt.y + dim.getIntY() - (int) (view.zoom * 2));
			g.drawLine(cnt.x + (int) (view.zoom * 2),
					cnt.y + dim.getIntY() / 2, cnt.x + dim.getIntX()
							- (int) (view.zoom * 2), cnt.y + dim.getIntY() / 2);
		}

		// ADDED Draw tooltip
		if (type.equals(TYPE_task) && property.selected
				&& taskDescriptionProvider != null) {
			String cleanName = getNameWithoutParameters();

			TaskDescription.Task taskDesc = taskDescriptionProvider
					.get(cleanName);
			if (taskDesc != null) {
	...
	Vec getSizeInternal() {
		return new Vec(getTextSize(view.graphics, font, getText())).add(
				new Vec(10, 10)).scale(view.zoom);
	}
	...
	public void onViewChange() {
		if (view.graphics == null)
			return;
		property.size = new Vec(
				getTextSize(view.graphics, font, getShortText())).add(new Vec(
				10, 10));
	}
	...
	class ModifyDialog extends JDialog {
		...
		public final void initUI() {
			...
			if (taskDescriptionProvider != null)
				txtName = new Java2sAutoTextField(
					taskDescriptionProvider.getNames()); // txtName.selectAll();
			else
			...
			cType = new JComboBox(new String[] { TYPE_sequenser, TYPE_selector,
					TYPE_task, TYPE_parallel, TYPE_switch });
			...
			txtTaskDescScroll = new JScrollPane(txtTaskDescAlgoritm);
			...
			OK.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					...
					try {
						getProperty().test_time = Integer.parseInt(txtDbgTime
								.getText());
						getProperty().test_result = Boolean
								.parseBoolean((String) txtDbgResult
										.getSelectedItem());
						...
						if (type == TYPE_task
							...
							updateTask.algorithm = txtTaskDescAlgoritm
									.getText();
							taskDescriptionProvider.put(
			...
			txtName.addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
					if (taskDescriptionProvider == null || descriptionLocked)
						return;

					String typedText = getNameWithoutParameters(txtName
							.getText());
					TaskDescription.Task taskDesc = taskDescriptionProvider
							.get(typedText);

					if (descriptionChanged && !txtTaskDescAlgoritm.getText().trim().equals("")) {
						
						// InputBox
						if (taskDesc != null) {
							int dialogResult = JOptionPane.showConfirmDialog(null, "Save description?", "Description has changed", JOptionPane.YES_NO_OPTION);
			...
			txtTaskDescAlgoritm.addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
			...
			txtTaskDescScroll.setPreferredSize(new Dimension(370, 150));
			txtTaskDescScroll.setBounds(160, 170, 370, 150);
			txtTaskDescScroll.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
			txtTaskDescAlgoritm.setLineWrap(true);
