 ...
 * The C22_GroundRecognitionAndMapping module goal is to provide a safe driving path defined as a polynomal
 *
...
public:
	 ...
	 */
	  C22_Node(std::string left_camera,std::string right_camera) :
		it_(nh_),
		...
		sync( MySyncPolicy( 10 ), left_image_sub_, right_image_sub_ )
	  {
		...
		sync.registerCallback( boost::bind( &C22_Node::callback, this, _1, _2 ) );  //Specifying what to do with the data
...
int main(int argc, char **argv)
{
  ...
  if(argc!=3){
	  printf("usage: C22_module <left camera topic> <right camera topic>");
  }
  C22_Node my_node(argv[1],argv[2]);
