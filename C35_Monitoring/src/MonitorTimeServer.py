...
class MonitorTimeServer(object):
	...
	def callback(self, data):
		...
		print "In callback, monitored node is:%s" % self._monitored_node_id
		if code == "1":		#This means a task is over
			print "Node with id=%s has ended." % node_id
			if self._monitored_node_id == node_id:
				print "The node we are monitoring has ended! Time:%f" % (time.time() - self._start_time)
...
#		task_result = TASK_RESULT_OK
		...
		rospy.Subscriber("/executer/stack_stream", String, self.callback)
           	...
           	while not rospy.is_shutdown() and time.time() - self._start_time  < threshold_time and not self._monitored_task_finished_on_time:
	            	rospy.sleep(0.2)
