...
class MonitorTimeServer(object):
	...
	def callback(self, data):
		...
		if code == "1":		#This means a task is over
			start_time = self._node_start_times_by_id.get(node_id, self._start_time)     #this assigns start_time with default value of the monitor's start time if the start message for this node did not arrive.
			print "Node with id=%s has ended after %f seconds" % (node_id, current_time - start_time)
			if self._monitored_node_id == node_id:    
				print "The node we are monitoring has ended! Time:%f" % (current_time - start_time)
				self._monitored_task_finished_on_time = True
		else:			#This means a task has started
			print "Node with id=%s has begun." % node_id
			self._node_start_times_by_id[node_id] = time.time()
   			print self._node_start_times_by_id
			if self._monitored_node_id == node_id: 
				print "The node we are monitoring has started! Time:%f" % self._node_start_times_by_id.get(node_id)
		...
		self._offline_computed_BT = xmlTree(sys.argv[0][0:-11]+event_file)
        	...
        	self._offline_computed_BT.createWrapperTreeMap("id") 
...
#		task_result = TASK_RESULT_OK
		...
		MonitoredNode = self._offline_computed_BT.getWrappedNode(self._monitored_node_id)
			...
			print "ERROR: Can't find monitored node with id:%s" % self._monitored_node_id
           	...
           	while not rospy.is_shutdown() and time.time() - self._node_start_times_by_id.get(self._monitored_node_id, self._start_time)  < threshold_time and not self._monitored_task_finished_on_time:
	            	rospy.sleep(0.2)
