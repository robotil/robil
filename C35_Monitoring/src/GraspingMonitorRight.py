...
#			  GraspingMonitor.detected_problem = True
	...
	def __init__(self, name):
		print ("Initializing grasping monitoring right Node")
		rospy.Subscriber("/sandia_hands/r_hand/tactile_raw", RawTactile, self.callback)
		GraspingMonitor.detected_problem = False
		GraspingMonitor.run_num = 0
		self.resultPublisher = rospy.Publisher("C35/grasp_force_r",String)
		RobilTask.__init__(self, name)
	
	def task(self, name, uid, parameters):
		print ("Start Monitoring grasping of the robot." )
		#print parameters
		d = rospy.Duration(1,0)
		
		GraspingMonitor.init_time = rospy.get_time()
		GraspingMonitor.started_task = True
		print ("Grasping monitor right received first message.")
		while not GraspingMonitor.detected_problem:	
			#print GraspingMonitor.detected_problem
			if self.isPreepted():
				print ("Preempt grasping right monitoring task")
				return RTResult_PREEPTED()
			rospy.sleep(d)
		print ("detected problem in grasping right module! EXITING...")
		GraspingMonitor.detected_problem = False
		error_code = RobilTask_FAULT + 1
		return RTResult_ABORT(error_code,"Grasping Monitor Right detected a problem. "+str(error_code));
		#return RTResult_SUCCESSED("Finished in Success")
      
if __name__ == '__main__':
	print ("Start GraspingMonitor Node")
	rospy.init_node('GraspingMonitorRight')
	GraspingMonitor("GraspingMonitorRight")
	rospy.spin()
	print ("Grasping monitor Right Node Closed") 
