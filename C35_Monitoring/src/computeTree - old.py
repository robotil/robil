...
def nodeDataInDebugMode(nodeTime, nodeSuccFail, nodeID, monitordNodeID, numOfIter):
   ...
   # print "prob="+str(Ctree.myTree.getWrappedNode(monitordNodeID).getProbAtIndex(0))
    if monitordNodeID == "":
        finished_node = Ctree.myTree.getRoot()
    else:
...
# Liat Here!  
    sd =Ctree.myTree.getWrappedNode(monitordNodeID).getSDSuccTime(0)
...
def getNodeInfo(nodeID):
    Ctree.myTree.createWrapperTreeMap("id")
    E =  Ctree.myTree.getWrappedNode(nodeID).getAverageSuccTime(0)
    prob = Ctree.myTree.getWrappedNode(nodeID).getProbAtIndex(0)
