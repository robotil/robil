...
bool C23_Detector::detectCar(Mat srcImg, const sensor_msgs::PointCloud2::ConstPtr &cloud) {
    ...
    return false;
	 ...
	 if(!res) {
	   ...
	   c21srv.request.sample.x1 = mcL[biggstL].x;
	   c21srv.request.sample.y1 = mcL[biggstL].y-10;
	   c21srv.request.sample.x2 = mcR[biggstR].x;
	   c21srv.request.sample.y2 = mcR[biggstR].y+10;
	 ...
	 //  c21srv.request.sample.y2 = mcR[biggstR].y+40;
	  ...
	  y = y1;
	   ...
	   Point2f b(x,y);
	   circle( srcImg, b, 16, Scalar(0,255,255), -1, 8, 0 );
	  imshow("TESTING",srcImg);
	   waitKey(0);
	   return true;
	 }
	 ...
	 if(right) {
	   ...
	   if(gateUpperDistance > 2.25 && gateUpperDistance < 2.9){      
	     ...
	     c21srv.request.sample.y1 = y;
	     c21srv.request.sample.x2 = x;
	     c21srv.request.sample.y2 = y;
