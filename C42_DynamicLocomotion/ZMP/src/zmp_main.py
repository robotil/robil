...
step_length =0# 0.05   # [m]
step_width  =0.178  # 0.1## [m]
step_time   = 2   # [sec]
bend_knees  = 0.02  # [m]    
time_step  =  0.01  # [sec] 
step_height = 0.08 # [m] 
...
p_ref1y = step_width*ones(( len(t1) ))   ###########step_width*ones(( len(t1) ))
p_ref0 = zeros(( len(t1)-1 ))
p_ref0_0x = zeros(( len(t0x)-1 ))
p_ref0_0y = (step_width/2)*ones(( len(t0y)-1 ))######(step_width/2)*ones(( len(t0y)-1 ))
...
while not rospy.is_shutdown():
  ...
  if ns.walk == 1:
      ...
      #p_ref_y = r_[ p_ref0_0y , sigmoid_y/2+step_width/2 ,p_ref1y,-sigmoid_y/2+step_width,p_ref0_0y,-sigmoid_y/2+step_width/2,-p_ref1y+step_width,sigmoid_y/2,p_ref0_0y]
      p_ref_y = r_[ p_ref0_0y , sigmoid_y/2+step_width/2 ,p_ref1y]
      #p_ref_y = r_[ p_ref0_0y , sigmoid_y/2+step_width/2 ,p_ref1y,-sigmoid_y/2+step_width,p_ref0_0y,-sigmoid_y/2+step_width/2,-p_ref1y+step_width,sigmoid_y/2,p_ref0_0y]
      #p_ref_y = r_[ p_ref0_0y , sigmoid_y/2+step_width/2 ,p_ref1y,-sigmoid_y+step_width,-p_ref1y+step_width,sigmoid_y,p_ref1y ]
  ...
  #######################################################
      ...
      if full_step:
         ...
         if(k>k_start_swing  and k<k_stop_swing ):
              swing_x_t_more_double_support = (k-k_start_swing)*step_length/(k - k_start_swing)
              swing_z_t_more_double_support = step_height*abs(sin(pi/(k_stop_swing -k_start_swing)*(k - k_start_swing)))  
         else:
              swing_z_t_more_double_support = 0
              swing_x_t_more_double_support = 0  
