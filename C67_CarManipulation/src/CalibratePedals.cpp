#ifndef __CALIBRATEPEDALS__HPP
#define __CALIBRATEPEDALS__HPP
...
class CalibratePedalsServer: public RobilTask {
protected:
	enum Consts { Time = 5 };
...
public:
	CalibratePedalsServer(std::string name, std::vector<string> par):
...
//		{
//			ROS_INFO("%s: No Angle Defined!", _name.c_str());
//			retValue  = NoParams;
//			return TaskResult(retValue, "ERROR");
//		}
//
//		angle = cast<double>(args["angle"]);
//		ROS_INFO("%s: Target Angle %f", _name.c_str(), angle);
...
{
	if(argc<0) return 1;
	std::string tname ("CalibratePedals");
	
	if(tname=="-h" || tname=="--help"){
		cout<<"CalibratePedals Server"<<endl;
		cout<<"Syntax: CalibratePedalsServer [ARG1=VALUE] [ARG2=VALUE] ..."<<endl;
		cout<<"Arguments:"<<endl;
	...
	stringstream snodename; snodename<<"CalibratePedalsServerNode_"<<time(NULL);
	...
	CalibratePedalsServer task(tname, params);
