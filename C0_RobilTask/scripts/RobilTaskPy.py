...
import roslib; roslib.load_manifest('C0_RobilTask')
import rospy
...
def RTResult_PREEPTED():
	return RTResult(RobilTask_FAULT, "", "Preempted", True)
def RTResult_SUCCESSED(desc):
	return RTResult(RobilTask_SUCCESS, "", desc, False)
def RTResult_PLAN(plan,desc):
	return RTResult(RobilTask_PLAN, "", desc, False)
def RTResult_ABORT(err,desc):
	if err<=RobilTask_SUCCESS:
		err = RobilTask_FAULT
...
class RobilTask(object):
	_result   = C0_RobilTask.msg.RobilTaskResult()

	def __init__(self, name):
		self._action_name = name
		self._goals = {}
		self._as = MGActionServer(self._action_name, C0_RobilTask.msg.RobilTaskAction, self.abstract_task, False)
	...
	def finish(self, res):
		...
		if res.isPreepted:
			...
			return
		if res.success<=RobilTask_SUCCESS:
			self._result.success = res.success
			if res.success == RobilTask_PLAN:
